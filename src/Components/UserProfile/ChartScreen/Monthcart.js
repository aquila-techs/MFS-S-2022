...
const Monthchart = ({ weight, graphdata, data }) => {
    ...
    fetch(`${API_URL}/user/getUserWeightList`, {
      ...
      .then((responseJson) => {
        ...
        responseJson.user.map((item) => {
          ...
          let check = moment(item.createdAt).locale(I18n.locale == undefined ? 'nl' : I18n.locale).format('dddd')
          days.push(check)
  ...
  if (loader == true) {
    ...
    return (
      <View style={styles.container}>
        <Text style={{ textAlign: "left" }}>{I18n.t('MonthChart')}</Text>
        <LineChart
          data={{
            labels: labels,
            datasets: [
