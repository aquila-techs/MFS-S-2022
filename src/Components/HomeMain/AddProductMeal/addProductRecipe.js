...
class AddProductRecipe extends Component {
  constructor(props) {
    super(props);
    this.state = {
      loader: false,
        ...
        item.mealType[0] == Recipe || item.mealType[1] == Recipe || item.mealType[2] == Recipe ?
          ...
          >
            <View style={styles.itemItem}>
              ...
              <View style={[styles.itemItemExtra, { marginTop: 2, marginBottom: 5 }]}>
                <Text style={[styles.itemItemTitle, { fontWeight: 'bold', fontSize: 15 }]} numberOfLines={1}>
                  {Engchecked ? item?.nameEn : item?.nameNl}
                </Text>
  ...
  checkCondition = async (item) => {
    ...
    if (this.state.referrelUse == 'false') {
      ...
      item.pricing == "free" || this.props?.userSubscription !== undefined ?
        this.props?.navigation.navigate("RecipesDetail", { blogItem: item, RECIPETYPE: this.props.route.params.RECIPETYPE })
        :
  ...
  componentDidMount = async () => {
    ...
    // const check = this.props.route.params.RECIPETYPE;
    // await AsyncStorage.setItem('RECIPETYPE', check)
